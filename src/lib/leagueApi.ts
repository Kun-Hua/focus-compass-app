...
export function getWeekStartISO(date = new Date(), timeZone: string = 'Asia/Taipei') {
  ...
  const day = tzNow.getDay() // 0..6 (Sun..Sat)
...
export async function getMyAnonymousBoard(weekStart?: string): Promise<AnonymousBoardRow[]> {
  const { data, error } = await (supabase.rpc as any)('get_my_group_anonymous_board', weekStart ? { p_week_start: weekStart } : undefined)
  if (error) throw error
...
export async function getMyBadges(): Promise<UserBadgeRow[]> {
  const { data, error } = await supabase
    .from('user_badges')
    .select('user_id,badge_id,granted_at,reason,badge:badge_id(badge_id,badge_code,badge_name,badge_desc)')
    .order('granted_at', { ascending: false })
  if (error) throw error
  return (data || []) as UserBadgeRow[]
}

// 手動觸發 Edge Function（僅用於測試與手動結算）
export async function invokeFocusLeagueEdge(): Promise<{ ok: boolean; week_start?: string; error?: string }> {
  console.log('[leagueApi] invokeFocusLeagueEdge: Triggered.')
  // 取得 anon key，確保它被正確載入
  ...
  // 關鍵修正：在呼叫 invoke 時，明確傳遞 Authorization 標頭。
  // 這會強制 supabase-js 使用 anon key 進行身份驗證，解決 401 錯誤。
...
export async function getLeagues(): Promise<League[]> {
  const { data, error } = await supabase.from('leagues').select('league_id,league_name,league_name_en').order('league_id', { ascending: true })
  if (error) throw error
...
export async function getMyMaskedId(weekStart?: string): Promise<string | undefined> {
  try {
    const { data, error } = await (supabase.rpc as any)('get_my_masked_id', weekStart ? { p_week_start: weekStart } : undefined)
    if (error) return undefined
    if (!data) return undefined
    // assume RPC returns { masked_id: string } or string
    if (typeof data === 'string') return data
    if (Array.isArray(data) && data.length > 0 && typeof data[0]?.masked_id === 'string') return data[0].masked_id
